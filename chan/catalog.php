...
# Echo the start of the comment box
echo("<div><form class=\"commentBox\" action=\"submit.php\" method=\"post\" enctype=\"multipart/form-data\" id=\"upload\"><select form=\"upload\" name=\"threadUpload\" id=\"threadUpload\"><option value=\"new\">New thread</option>");
# For each thread in the main thread array
for($x=1;$x<=count($displayThreads);$x++) {
	# Add an option to the reply list
	echo("<option value=\"{$displayThreads[$x]['thread']}\">Thread #{$displayThreads[$x]['thread']}</option>");
}
# Echo the rest of the comment box
